...
class AccountShow extends React.Component {
  ...
  componentDidMount() {
    fetch(
      `http://localhost:3001/api/v1/accounts/${this.props.routerProps.match.params.id}`
    )
      .then((res) => res.json())
      .then((data) =>
        this.setState({
      ...
      return (
        <div>
          ...
          <a
            ...
            target="_blank"
            href={this.state.account.website}
          >
            <p>
          ...
          <Fab
            onClick={() =>
              this.nextPath(`/accounts/${this.state.account.id}/edit`)
            }
            color="secondary"
            aria-label="add"
          >
