...
class ContactShow extends React.Component {
  ...
  componentDidMount() {
    fetch(
      `http://localhost:3001/api/v1/contacts/${this.props.routerProps.match.params.id}`
    )
      .then((res) => res.json())
      .then((data) =>
        this.setState({
  ...
  render() {
    if (this.state.emailFormOpen) {
      return (
        <MessageForm
          emailFormOpen={this.state.emailFormOpen}
          contact={this.state.contact}
          handleToggleEmailForm={this.handleToggleEmailForm}
        />
      ); // eslint-disable-line
    } else {
      if (this.state.contact) {
        return (
          <div>
            ...
            <Button
              onClick={this.handleToggleEmailForm}
              variant="contained"
              color="primary"
            >
              Send Email
            ...
            <Fab
              onClick={() =>
                this.nextPath(`/contacts/${this.state.contact.id}/edit`)
              }
              color="secondary"
              aria-label="add"
            >
