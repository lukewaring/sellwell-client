...
class ContactShow extends React.Component {
    ...
    render () {
      if (this.state.emailFormOpen) {
        return <MessageForm emailFormOpen={this.state.emailFormOpen} contact={this.state.contact} handleToggleEmailForm={this.handleToggleEmailForm} /> // eslint-disable-line
      } else {
        if (this.state.contact) {
          return (
            <div>
              <h2 style={{ textAlign: 'center' }}>{this.state.contact.name}</h2>
              <Button onClick={this.handleToggleEmailForm} variant='contained' color='primary'>Send Email</Button>

              <Button
                onClick={() => window.confirm('Are you sure you want to delete this contact?') && this.deleteContact()}
                variant='contained'
                style={style}
